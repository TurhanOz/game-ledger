...
function submitEdit() {
  ...
  if (name) store.updatePlayer(editId.value, name)
  editId.value = null
  editName.value = ''
}
...
function confirmDelete() {
  ...
  try {
    store.deletePlayer(deleteTarget.value)
  } catch {
    errorMessage.value = 'Cannot delete: player is linked to an existing session or refund.'
  }
...
<template>
  <div>
    <h2 class="text-lg font-semibold text-gray-800 mb-3">Players</h2>
    ...
    <ul class="space-y-2">
      ...
      >
        <!-- Edit mode -->
        <template v-if="editId === player.id">
          <input
            v-model="editName"
            type="text"
        ...
        <!-- View mode -->
        <template v-else>
          <span class="flex-1 text-sm text-gray-800">{{ player.name }}</span>
          <button
            class="text-sm text-indigo-600 hover:underline min-h-[44px] px-2"
    ...
    <p v-if="store.players.length === 0" class="text-sm text-gray-400 italic mt-2">
      No players yet. Add one above.
    </p>

    <ConfirmDialog
      :open="deleteTarget !== null"
      message="Delete this player? This action cannot be undone."
